...
/// 데모 모드 여부 (Supabase 연결 실패 시 true, 또는 기본값이면 true)
bool isDemoMode = false;

/// Supabase가 기본값인지 확인
bool _isDefaultSupabaseConfig() {
  return Env.supabaseUrl == 'https://your-project.supabase.co' ||
...
void main() async {
  ...
  // Supabase 설정이 기본값이면 데모 모드
  if (_isDefaultSupabaseConfig()) {
    debugPrint('Supabase 설정이 기본값입니다. 데모 모드로 실행합니다.');
    isDemoMode = true;
    // 데모 모드에서도 Supabase 초기화 시도 (실패해도 무관)
    try {
      ...
      debugPrint('Supabase 초기화 실패 (데모 모드): $e');
    }
  } else {
...
class MoeBackstageApp extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    // 데모 모드에서도 라우터 사용 (로그인 페이지에서 데모 로그인 가능)
